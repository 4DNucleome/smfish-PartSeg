...
def verify_sm_segmentation(
    ...
    for i, points_group in enumerate(points_grouped):
        ...
        if ignore_single_points and len(points_group) == 1 and not result.matched_points[i]:
            result.matched_points[i] = True
            result.ignored += 1
...
def verify_segmentation(
    ...
    res = []
    for ok, points_group in zip(match_result.matched_points, match_result.points_grouped):
        if not ok:
